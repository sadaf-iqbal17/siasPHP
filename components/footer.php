<footer>
<!----Footer-->
    <div class="content">
      <div class="top">
        <div class="logo-details">
          <i class="fab fa-slack"></i>
          <span class="logo_name">SIAS</span>
        </div>
        <div class="media-icons">
          <a href="#"><i class="fab fa-facebook-f"></i></a>
          <a href="#"><i class="fab fa-twitter"></i></a>
          <a href="#"><i class="fab fa-instagram"></i></a>
          <a href="#"><i class="fab fa-linkedin-in"></i></a>
          <a href="#"><i class="fab fa-youtube"></i></a>
        </div>
      </div>
      <div class="link-boxes">
        <ul class="box">
            <li class="link_name">About Us</li>
           <p id="about">We are a team of undergraduate students going to develop an E-learning website based on the technologies of computer vision and machine learning.</p>
          </ul>

        <ul class="box">
          <li class="link_name">Useful Links</li>
          <li><a href="home.php">Home</a></li>
          <li><a href="contact.php">Contact us</a></li>
          <li><a href="about.php">About us</a></li>
          <li><a href="#">Get started</a></li>
        </ul>

        <ul class="box">
          <li class="link_name">Courses</li>
          <li><a href="courses.php">HTML & CSS</a></li>
          <li><a href="courses.php">JavaScript</a></li>
          <li><a href="courses.php">Photography</a></li>
          <li><a href="courses.php">Photoshop</a></li>
        </ul>

        <ul class="box input-box">
          <li class="link_name">Subscribe</li>
          <li><input type="text" placeholder="Enter your email"></li>
          <li><input type="button" value="Subscribe"></li>
        </ul>
      </div>
    </div>
    <div class="btm_footer">

      &copy; copyright @ <?= date('Y'); ?> by <span>Sadaf</span> | all rights reserved!
    </div>
  

   

</footer>